 ...
 */
//! [0]
ImageLoader::ImageLoader(const QString &imageUrl, const QString &rom, QObject* parent)
    : QObject(parent)
    , m_loading(false)
    , m_imageUrl(imageUrl)
	, m_rom(rom)
...
void ImageLoader::load()
{
    ...
    //Check if m_imageUrl after last '/' exists in shared/misc/pcsx-rearmed-pb/boxart
    //if so then skip the following and call onImageProcessingFinished after loading QImage

    //QImage image = QImage(QString("shared/misc/pcsx-rearmed-pb/boxart/%1").arg(m_rom.mid(m_rom.lastIndexOf("/"), m_rom.indexOf("("))));
    ...
    //if(image.isNull()){
		...
		//qDebug() << "Load Image...";
    //} else {
    //	onImageProcessingFinished(image);
    //}
}
//! [2]
...
void ImageLoader::onReplyFinished()
{
    ...
    if (reply) {
        if (reply->error() == QNetworkReply::NoError) {
        	//qDebug() << "Load Image No Error...";
            const int available = reply->bytesAvailable();
            if (available > 0) {
                const QByteArray data(reply->readAll());

                //qDebug() << "Before: " << m_rom;
                int tmp = m_rom.indexOf("(");
                QString name;
                if(tmp == -1){
                	name = m_rom.mid(m_rom.lastIndexOf("/")+1, m_rom.lastIndexOf(".")-m_rom.lastIndexOf("/")-1);
                } else {
                	name = m_rom.mid(m_rom.lastIndexOf("/")+1, tmp-m_rom.lastIndexOf("/")-1);
                }
                //qDebug() << "After: " << name;

                // Setup the image processing thread
                ImageProcessor *imageProcessor = new ImageProcessor(data, name.trimmed());
    	...
    	//qDebug() << "Load Image Error No Internet...";
...
QVariant ImageLoader::setImage(QVariant &image)
{
    //m_image = image.convert();
    return 0;
}
